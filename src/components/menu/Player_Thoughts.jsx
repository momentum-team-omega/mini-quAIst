...
const Player_Thoughts = ({ charPosition }) => {
  ...
  useEffect(() => {
    ...
    const isWithinRange2 =
        charPosition.x >= 2.5 && charPosition.x <= 4.5 && charPosition.y >= 7 && charPosition.y <= 9;
    ...
    switch (currentMap) {
        case "startHouse":
            ...
            break;
        case "start":
            if (charPosition.y >= 8 && charPosition.y <= 10 && charPosition.x >= 16.5) {
                tempContent = "I think my cat went this way...";
            }
            break;
        case "enchantedForestLocked":
            if (isWithinRange1) {
                tempContent = "This path looks blocked. I wonder if someone could help me get through...";
            } else if (isWithinRange2) {
                tempContent = "I hear someone. Maybe I should go talk to them...";
            }
            break;
        case "village2Locked":
            if (charPosition.x <= 4 && charPosition.y >= 18 && charPosition.y <= 24) {
            ...
            } else if (charPosition.x >= 18.5 && charPosition.x <= 21.5 && charPosition.y >= 8 && charPosition.y <= 13) {
                tempContent = "The town hall says closed for lunch. I wonder if I should talk to steve in the meantime...";
            }
            break; 
